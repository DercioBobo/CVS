...
function update_profileview($user_id){
    ...
    if($person['view'] != null){
        $query = "UPDATE `".$config['db']['pre']."user` SET `view` = 'view+1' WHERE `id` = '".validate_input($user_id)."' LIMIT 1 ";
    }else{
        $query = "UPDATE `".$config['db']['pre']."user` SET `view` = '1' WHERE `id` = '".validate_input($user_id)."' LIMIT 1 ";
    }
    $pdo->query($query);
}
