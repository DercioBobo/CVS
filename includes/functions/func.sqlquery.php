...
function get_customFields_by_catid($maincatid=null,$subcatid=null,$require=true,$fields=array(),$data=array()){
    ...
    foreach ($result as $info)
    {
        ...
        if($info['custom_type'] == 'radio-buttons')
        {
            ...
            $radiobtn .= '<input type="hidden" class="quick-radioCheck" data-name="'.$info['custom_id'].'" data-req="'.$required.'"><div class="quick-error">'.$lang['FIELD_REQUIRED'].'</div>';
                ...
                if($config['tpl_name'] == 'thenext-theme'){
                    ...
                    $Checkbox .= '
                        <div class="checkbox checkbox-inline checkbox-primary">
                            <input type="checkbox" name="custom['.$info['custom_id'].'][]" id="'.$value4.$j.'" value="'.$value4.'" data-name="'.$info['custom_id'].'" '.$selected.' />
                            <label for="'.$value4.$j.'" >'.$option_title.'</label>
                        </div>';
                }
                $j++;
            }
            $Checkbox .= '<input type="hidden" class="quick-radioCheck" data-name="'.$info['custom_id'].'" data-req="'.$required.'"><div class="quick-error">'.$lang['FIELD_REQUIRED'].'</div>';
...
u.group_id,u.status as user_status, g.show_on_home
...
$where ORDER BY $order_by $pagelimit";
    ...
    if ($result) {
        foreach ($result as $info)
        {
            ...
            $item[$info['id']]['user_status'] = $info['user_status'];
...
function payment_fail_save_detail($access_token){
    ...
    if($payment_type == "subscr"){
        ...
        $last_id = $trans_insert->id();
        $_SESSION['quickad'][$access_token]['transaction_id'] = $last_id;
    }
    else{
        $item_pro_id = $_SESSION['quickad'][$access_token]['product_id'];
